<?php 
	...
	//----------------------CONTROLAR SI LA BOLETA TIENE UN MEDIO DE PAGO CHEQUE INSERTAR EN CABECERA RUT SI NO 1-9----------------------
	$rutBoleta=$jsonBoletaCabecera['rutCliente'];
	$jsonBoletaCabecera['rutCliente']='';
	for($i=0;$i<count($jsonBoletaPagos['tipoPago']);$i++){
		if($jsonBoletaPagos['tipoPago'][$i] == 'Check' || $jsonBoletaPagos['tipoPago'][$i] == 'Payments'){
			$jsonBoletaCabecera['rutCliente']=$rutBoleta;
			$jsonBoletaCabecera['rutDespacho'] =$rutBoleta;
		}
	}
	//---------------------------------------------------------------------------------------------------------------------
	...
	if($resultadoNoRepeticion['numeroDocto'] != null || $resultadoNoRepeticion['numeroDocto'] != ''){
			...
			while($resultado = odbc_fetch_array($rsCIFLotes)){
				...
				while($cantAux>0){
					if($cantAux<(int)$resultado['Cantidad']){
						...
						//INSERTAR DETALLE
						$sqlInsertarDetalle = " IF NOT EXISTS (SELECT * FROM RP_VICENCIO.dbo.RP_ReceiptsDet_SAP WHERE ID ='".$jsonBoletaDetalle['ID']."' AND secuencia ='".$secuencia."')
		...
		//--------------------------------------------------------------------------------------------------------------------------------------------------
		echo 1;	//Resultado de respuesta a control de repetición de la boleta	
	}
	odbc_close( $conn );
?>